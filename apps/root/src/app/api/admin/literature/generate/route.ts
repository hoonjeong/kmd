...
import { getClaudeClient, buildLiteratureQuestionPrompt } from '@/lib/claude-client';
...
export async function POST(req: NextRequest) {
  ...
  const creditDesc = `문학 문제 생성 (${creditCount}문항)`;
  ...
  try {
    // 전체 조건으로 검색
    let passageSets = await selectLiteraturePassageSets({
      title: body.title,
      author: body.author,
    ...
    // 결과 없으면 제목/작가만으로 재검색
    if (passageSets.length === 0 && (body.title || body.author)) {
      passageSets = await selectLiteraturePassageSets({
        title: body.title,
        author: body.author,
        passageLimit: 5,
      });
    ...
    const prompt = buildLiteratureQuestionPrompt(passageSets, {
      subCategories: body.subCategories,
      questionPatterns: body.questionPatterns,
      title: body.title,
      author: body.author,
