...
export default function ReadingClient({
  ...
  const fetchFilters = useCallback(async (opts: {
    ...
    if (opts.kw) sp.set('keyword', opts.kw);
    const qs = sp.toString();
    try {
      const res = await fetch(`/api/admin/reading/filters${qs ? '?' + qs : ''}`);
  ...
  function refreshCount(subCats: string[], patterns: string[]) {
    ...
    fetchFilters({ subCats, patterns, kw: keywordRef.current.trim() || undefined });
  }
  ...
  async function handleSearch() {
    ...
    setPreviewSets(null);
    try {
      const res = await fetch('/api/admin/reading/search', {
        method: 'POST',
  ...
  function handleGenerate() {
    generate('/api/admin/reading/generate', {
      keyword: keyword.trim() || undefined,
      subCategories: selectedSubCategories.length > 0 ? selectedSubCategories : undefined,
      questionPatterns: selectedPatterns.length > 0 ? selectedPatterns : undefined,
      difficulty,
  ...
  const downloadTitle = keyword.trim() ? `${keyword.trim()}_변형문제` : '독서_변형문제';
      ...
      <div className="card mb-3">
        <div className="card-body">
          ...
          <div className="form-group" style={{ marginBottom: 22 }}>
            <label style={{ fontWeight: 600, marginBottom: 6, display: 'block' }}>키워드</label>
            <input type="text" className="form-control" value={keyword}
              onChange={e => setKeyword(e.target.value)}
              placeholder="경제, 과학, 인문 등 검색어 입력" disabled={isGenerating} />
          </div>
              ...
              <div style={{ display: 'flex', gap: 16, paddingTop: 4 }}>
                {DIFFICULTY_OPTIONS.map(opt => (
                  <label key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: 4, cursor: 'pointer', fontSize: 14 }}>
                    <input type="radio" name="read-difficulty" value={opt.value}
            ...
            {previewSets.length === 0 ? (
              ...
              <div key={setIdx} style={{
                ...
                <div style={{ marginBottom: 8 }}>
                  <span style={{ display: 'inline-block', background: '#dcfce7', color: '#166534', fontSize: 12, fontWeight: 600, padding: '2px 8px', borderRadius: 4, marginRight: 8 }}>
                    지문 {setIdx + 1}
                  </span>
                  <span style={{ fontSize: 13, color: '#64748b' }}>
                    {set.passage?.subCategory || ''}
                    {set.passage?.keywords ? ` — ${set.passage.keywords}` : ''}
                  </span>
                </div>
                {set.passage?.content && (
